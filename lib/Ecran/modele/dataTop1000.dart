...
class DataTop1000 {
  ...
  Future SelectNombreAttente(int id) async {
    ...
    await db.rawUpdate("UPDATE preparation SET etat_Attente = 1 WHERE id_prep = $id ");
  }
  ...
  Future SelectNombreValider(int id) async {
    ...
    if (count == 0) {
      await db.rawUpdate("UPDATE preparation SET etat = 1 WHERE id_prep = $id ");
    } else {
      await db.rawUpdate("UPDATE preparation SET etat_Attente = 1 WHERE id_prep = $id ");
    }
  }
  ...
  Future SelectNombreAnnuler(int id) async {
    ...
    if (count == 0 && count2 == 0) {
      await db.rawUpdate("UPDATE preparation SET etat_attente = 0 WHERE id_prep = $id ");
    } else {
      await db.rawUpdate("UPDATE preparation SET etat_Attente = 1 WHERE id_prep = $id ");
    }
  }
  ...
  Future<Map<String, dynamic>> SelectLibelle(String gencode) async {
    ...
    List<Map<String, Object?>> result =
        await db.rawQuery("SELECT id_releve, libelle_art_conc FROM releve WHERE gencode_art_conc = '$gencode' AND prix_art_conc = 0  ");
    //List<Map<String, dynamic>> resulti = await db.query("Article", where: "", orderBy: "is ASC");
  ...
  Future UpdateEtatReleve(int id_releve, int id_prep) async {
    Database db = await DatabaseHelper().database;

    await db.rawUpdate("UPDATE releve SET etat_art = 1 WHERE id_releve = $id_releve AND id_prep = $id_prep ");
...
// prix
  Future AnnulerReleve(int id_releve, int id_prep) async {
    Database db = await DatabaseHelper().database;
    await db.rawUpdate(
        "UPDATE releve SET prix_art_conc= 0  , date_maj_releve = '${DateFormat("yyyy-MM-dd HH:mm:ss").format(DateTime.now())}' WHERE id_releve = $id_releve AND id_prep = $id_prep ");
  ...
  Future<List<Top1000>> SearchAttente(int id, String txt) async {
    ...
    List<Map<String, dynamic>> result = await db.rawQuery(
        "SELECT * FROM releve WHERE id_prep = ${id} AND etat_art = 0 AND prix_art_conc != 0 AND (ref_art LIKE '%$txt%' OR libelle_art LIKE '%$txt%' OR gencode_art LIKE '%$txt%' OR prix_art LIKE '%$txt%' OR ref_art_conc LIKE '%$txt%' OR libelle_art_conc LIKE '%$txt%' OR gencode_art_conc LIKE '%$txt%' OR prix_art_conc LIKE '%$txt%' OR date_val_releve LIKE '%$txt%' )  ");
    //List<Map<String, dynamic>> resulti = await db.query("Article", where: "", orderBy: "is ASC");
    result.forEach((MapElement) {
      Top1000 article = Top1000();
      article.fromMap(MapElement);
